    ...
    def __init__(self, skills_db_path: Optional[str] = None):
        ...
        for model_name, description in models_to_try:
            try:
                ...
                logger.info(f"✅ Modèle spaCy chargé : {model_name} ({description})")
                break  # Arrêter dès qu'un modèle fonctionne
            except OSError:
                logger.warning(f"⚠️  Modèle {model_name} non trouvé, essai suivant...")
                continue
        ...
        logger.info(f"✅ Base de compétences chargée depuis {skills_db_path.name}")
        logger.info(f"   • Compétences techniques : {len(self.skills_database['technical_skills'])}")
        logger.info(f"   • Soft skills : {len(self.skills_database['soft_skills'])}")
